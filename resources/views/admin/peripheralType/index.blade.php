<x-layout>
    <div class="ContentArea">
        ...
        <div class="TextImage" style="display: flex; justify-content: space-between; flex-wrap: wrap;">
            ...
            <a href="{{ route('peripheral-type.create') }}" class="Button color-border-white size-large"
                style="margin-bottom: 8px">
            ...
            <div class="TextImage--content richtext" style="overflow-x: scroll">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @foreach ($peripheralTypes as $peripheralType)
                            <tr>
                                <td style="text-align: center">
                                    <a href="{{ route('peripheral-type.edit', $peripheralType->id) }}" class="quickaction-anchor edit">
                                ...
                                <td style="text-align: center">
                                    <form class="deleteForm" data-name="{{ $peripheralType->name }}" action="{{ route('peripheral-type.destroy', $peripheralType->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @endforeach
                        @if ($peripheralTypes->isEmpty())
                            <tr>
                                <td colspan="3">No Peripheral Types found.</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
            </div>
            {{ $peripheralTypes->withQueryString()->links('pagination.uzh-pagination-en') }}
        </div>
    </div>
</x-layout>
<script>
    $(document).ready(function() {
        ...
        $('.delete').on('click', function() {
            ...
            // opens confirmation modal
            customConfirm('Are you sure you want to delete the peripheral type <b style="font-weight: bold;">' + name + '</b>?')
