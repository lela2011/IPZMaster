<x-layout>
    <div class="ContentArea">
        ...
        <div class="TextImage" style="display: flex; justify-content: space-between; flex-wrap: wrap;">
            ...
            <a href="{{ route('peripheral.create') }}" class="Button color-border-white size-large"
                style="margin-bottom: 8px">
                Add new Peripheral
        ...
        <form class="js-Form Form" action="{{ route('peripheral.index') }}" method="GET">
        ...
        <div class="TextImage">
                    ...
                    <tbody>
                        @foreach ($peripherals as $peripheral)
                            <tr>
                                ...
                                <td style="text-align: center">
                                    <form class="deleteForm" data-type="{{ $peripheral->type->name ?? "Mobile Device" }}" data-model="{{ $peripheral->model }}" action="{{ route('peripheral.destroy', $peripheral->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <td>{{ optional($peripheral->type)->name }}</td>
                                <td>{{ optional($peripheral->manufacturer)->name }}</td>
                                ...
                                <td>{!! nl2br(e($peripheral->notes)) !!}</td>
                                <td>
                                    @if ($peripheral->invoice)
                                        <a href="{{ route('admin.inventory.invoice.download', $peripheral->invoice) }}" target="_blank">View Invoice</a>
                                    @endif
                                </td>
                                <td>{{ optional($peripheral->supplier)->name }}</td>
                                <td>{{ optional($peripheral->person)->first_name }} {{ optional($peripheral->person)->last_name }}</td>
                            </tr>
                        @endforeach
            ...
            {{ $peripherals->withQueryString()->links('pagination.uzh-pagination-en') }}
        </div>
    </div>
</x-layout>
<script>
