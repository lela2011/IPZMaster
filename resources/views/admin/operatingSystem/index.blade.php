<x-layout>
    <div class="ContentArea">
        ...
        <div class="TextImage" style="display: flex; justify-content: space-between; flex-wrap: wrap;">
            ...
            <a href="{{ route('operating-system.create') }}" class="Button color-border-white size-large"
                style="margin-bottom: 8px">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @foreach ($operatingSystems as $operatingSystem)
                            <tr>
                                <td style="text-align: center">
                                    <a href="{{ route('operating-system.edit', $operatingSystem->id) }}" class="quickaction-anchor edit">
                                ...
                                <td style="text-align: center">
                                    <form class="deleteForm" data-name="{{ $operatingSystem->name }}" action="{{ route('operating-system.destroy', $operatingSystem->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @if ($operatingSystems->isEmpty())
                            <tr>
                                <td colspan="3">No Operating Systems found.</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
            </div>
            {{ $operatingSystems->withQueryString()->links('pagination.uzh-pagination-en') }}
        </div>
    </div>
</x-layout>
<script>
    $(document).ready(function() {
        ...
        $('.delete').on('click', function() {
            ...
            // opens confirmation modal
            customConfirm('Are you sure you want to delete the operating system <b style="font-weight: bold;">' + name + '</b>?')
