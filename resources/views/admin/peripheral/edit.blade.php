<x-layout>
    <div class="ContentArea">
        ...
        <div class="TextImage" style="display: flex; justify-content: space-between; flex-wrap: wrap;">
            <a href="{{ route('peripheral.index') }}" class="Button color-border-white size-large"
        ...
        <div class="TextImage">
            <h2 class="TextImage--title richtext">
                Update the Peripheral: {{ $peripheral->model ?? 'Unknown Model' }}
            </h2>
        </div>
        <form class="js-Form Form" action="{{ route('peripheral.update', $peripheral->id) }}" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PUT')
            <div class="Form--body">
                <div class="FormInput">
                    ...
                    <select class="selectFilter" name="type_id" id="type_id">
                        ...
                        @foreach ($types as $type)
                            <option value="{{ $type->id }}" @if($type->id == old('type_id', $peripheral->type_id)) selected @endif>{{ $type->name }}</option>
                        @endforeach
                    </select>
                    @error('type_id')
                ...
                <div class="FormInput">
                    ...
                    <select class="selectFilter" name="manufacturer_id" id="manufacturer_id">
                        ...
                        @foreach ($manufacturers as $manufacturer)
                            <option value="{{ $manufacturer->id }}" @if($manufacturer->id == old('manufacturer_id', $peripheral->manufacturer_id)) selected @endif>{{ $manufacturer->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="FormInput">
                    ...
                    <input class="Input" type="text" name="model" id="model" value="{{ old('model', $peripheral->model) }}">
                    @error('model')
                    <p class="has-error" style="color: red">
                        {{$message}}
                    </p>
                ...
                <div class="FormInput">
                    ...
                    <input class="Input" type="text" name="serial_number" id="serial_number" value="{{ old('serial_number', $peripheral->serial_number) }}">
                    @error('serial_number')
                    <p class="has-error" style="color: red">
                        {{$message}}
                    </p>
                ...
                <div class="FormInput">
                    ...
                    <input class="Input" type="text" name="product_number" id="product_number" value="{{ old('product_number', $peripheral->product_number) }}">
                ...
                <div class="FormInput">
                    ...
                    <select class="selectFilter" name="location_id" id="location_id">
                        ...
                        @foreach ($locations as $location)
                            <option value="{{ $location->id }}" @if($location->id == old('location_id', $peripheral->location_id)) selected @endif>{{ $location->name }}</option>
                        @endforeach
                    </select>
                    @error('location_id')
                    <p class="has-error" style="color: red">
                ...
                <div class="FormInput">
                    ...
                    <input class="Input" type="date" name="purchase_date" id="purchase_date" value="{{ old('purchase_date', $peripheral->purchase_date) }}">
                    @error('purchase_date')
                    <p class="has-error" style="color: red">
                        {{$message}}
                    </p>
                ...
                <div class="FormInput">
                    ...
                    <input class="Input" type="date" name="warranty_date" id="warranty_date" value="{{ old('warranty_date', $peripheral->warranty_date) }}">
                    @error('warranty_date')
                    <p class="has-error" style="color: red">
                        {{$message}}
                    </p>
                ...
                <div class="FormInput">
                    ...
                    <textarea class="Input" type="text" name="notes" id="notes">{!! e(old('notes', $peripheral->notes)) !!}</textarea>
                    @error('notes')
                    <p class="has-error" style="color: red">
                        {{$message}}
                    </p>
                ...
                <div class="FormInput">
                    ...
                    @if ($peripheral->invoice)
                        <div id="current_invoice" style="display: flex; justify-items: center; align-items: center; font-size: 0.875rem; line-height: 1.5; font-weight: 400; color: #666666; margin-bottom: 8pt">
                            <a href="{{ route('admin.inventory.invoice.download', $peripheral->invoice) }}" target="_blank" style="display: flex; align-items: center; margin-right: 8pt">
                                <span class="material-icons" style="margin-right: 4pt">
                                    open_in_new
                                </span>
                                <span>
                                    {{ basename($peripheral->invoice) }}
                                </span>
                            </a>
                ...
                <div class="FormInput">
                    ...
                    <select class="selectFilter" name="supplier_id" id="supplier_id">
                        ...
                        @foreach ($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" @if($supplier->id == old('supplier_id', $peripheral->supplier_id)) selected @endif>{{ $supplier->name }}</option>
                        @endforeach
                    </select>
                    @error('supplier_id')
                    <p class="has-error" style="color: red">
                ...
                <div class="FormInput">
                    ...
                    <select class="selectFilter" name="user_id" id="user_id">
                        ...
                        @foreach ($people as $person)
                            <option value="{{ $person->uid }}" @if($person->uid == old('user_id', $peripheral->user_id)) selected @endif>{{ $person->first_name }} {{ $person->last_name }}</option>
                        @endforeach
                    </select>
                    @error('user_id')
                    <p class="has-error" style="color: red">
