<x-layout>
    <div class="ContentArea">
        ...
        <div class="TextImage" style="display: flex; justify-content: space-between; flex-wrap: wrap;">
            ...
            <a href="{{ route('keyboard-layout.create') }}" class="Button color-border-white size-large"
                style="margin-bottom: 8px">
                        ...
                        @foreach ($keyboardLayouts as $keyboardLayout)
                            <tr>
                                <td style="text-align: center">
                                    <a href="{{ route('keyboard-layout.edit', $keyboardLayout->id) }}" class="quickaction-anchor edit">
                                ...
                                <td style="text-align: center">
                                    <form class="deleteForm" data-name="{{ $keyboardLayout->name }}" action="{{ route('keyboard-layout.destroy', $keyboardLayout->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <td>{{ $keyboardLayout->name }}</td>
                            </tr>
                        @endforeach
                        @if ($keyboardLayouts->isEmpty())
                            <tr>
                                <td colspan="3">No Keyboard Layouts found.</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
            </div>
            {{ $keyboardLayouts->withQueryString()->links('pagination.uzh-pagination-en') }}
        </div>
    </div>
</x-layout>
<script>
    $(document).ready(function() {
        ...
        $('.delete').on('click', function() {
            ...
            // opens confirmation modal
            customConfirm('Are you sure you want to delete the keyboard layout <b style="font-weight: bold;">' + name + '</b>?')
